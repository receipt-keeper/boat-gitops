...
`receipt-keeper/boat-gitops`는 Boat backend를 Argo CD로 배포하기 위한 GitOps
...
charts/boat-backend/          # Boat backend Helm chart
```
...
- `boat-backend-app`, `boat-backend-db`, `boat-firebase`, `ghcr-secret`은 외부에서
  미리 주입되는 입력으로 본다.
- DB migration은 앱 컨테이너 시작 명령에 넣지 말고 Argo CD `PreSync` Job hook으로
  관리한다.
- dev의 기존 live 리소스 채택을 위해 기본 리소스 이름은 `boat-backend`,
  `boat-postgresql`, `boat-backend-migrate`를 유지한다.
- StatefulSet selector와 PVC template 이름은 신중하게 다룬다. 특히
  `volumeClaimTemplates.metadata.name=data`를 바꾸면 기존
  `data-boat-postgresql-0` PVC와 달라진다.

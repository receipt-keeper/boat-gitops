...
|-- deploy/prod/              # prod Compose, Nginx, systemd, scripts
...
- `deploy/prod/image-tag`와 `image-digest` 변경 PR을 먼저 병합하고 같은 tag를 workflow에 입력한다.
...
서버 준비, Secret 계약, 배포와 rollback 절차는
[`docs/ops/production.md`](docs/ops/production.md)를 따른다.
